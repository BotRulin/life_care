...
class LifeCareSupabaseUser extends BaseAuthUser {
  LifeCareSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> lifeCareSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LifeCareSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
